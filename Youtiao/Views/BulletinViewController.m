...
//#import "BulletinOperationsTableViewCell.h"
...
- (void)viewDidLoad {
  ...
  self.commentNewViewModel = [self.bulletinViewModel commentNewViewModel];
...
- (void)bindViewModels {
  ...
  @weakify(self);
  [self.bulletinViewModel.fetchCommentsCommand.executionSignals subscribeNext:^(RACSignal *signal) {
  ...
  [self.commentNewViewModel.createCommentCommand.executionSignals subscribeNext:^(RACSignal *signal) {
    [signal subscribeNext:^(id x) {
      ...
      [self.bulletinViewModel.fetchCommentsCommand execute:nil];
    }];
  }];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  return [self.bulletinViewModel numberOfComments] + 1;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.row == 0) {
    ...
    ((BulletinDetailTableViewCell *)cell).bulletinViewModel = self.bulletinViewModel;
  } else {
    cell = [tableView dequeueReusableCellWithIdentifier:@"CommentCell"];
    ((CommentTableViewCell *)cell).commentViewModel = [self.bulletinViewModel commentViewModelAtIndex:indexPath.row-1];
  }
  return cell;
}
