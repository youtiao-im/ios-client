...
- (void)bindViewModels {
  ...
  [[self.groupViewModel.fetchMembershipsCommand execute:nil] subscribeCompleted:^{
    [self.membershipsTableView reloadData];
  }];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  return [self.groupViewModel numberOfMemberships];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  MembershipViewModel *membershipViewModel = [self.groupViewModel membershipViewModelAtIndex:indexPath.row];
